...
void cali_data_read(void)
{
    ...
    {
        //读取校准设备的前部数据
        cali_flash_read(ADDR_FLASH_SECTOR_10 + offset, (int32_t *)flash_read_buf, CALI_SENSOR_HEAD_LEGHT);
        //将名字，校准标识符进行赋值
        ...
        cali_flash_read(ADDR_FLASH_SECTOR_10 + offset, (int32_t *)cali_sensor[i].flash_buf, cali_sensor[i].flash_len);
        //偏移校准数据字节大小
...
static void cali_data_write(void)
{
    ...
    //写入flash
    cali_flash_write(ADDR_FLASH_SECTOR_10, (int32_t *)buf, len);
}
